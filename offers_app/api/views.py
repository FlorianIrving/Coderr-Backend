...
from offers_app.api.serializers import OfferGetDetailSerializer
...
class OfferDetailView(APIView):
    ...
    def get(self, request, pk):
        ...
        serializer = OfferGetDetailSerializer(offer, context={"request": request})
        return Response(serializer.data, status=200)
    
    def patch(self, request, pk):
        offer = get_object_or_404(Offer, pk=pk)
        serializer = OfferGetDetailSerializer(offer, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=200)
        return Response(serializer.errors, status=400)